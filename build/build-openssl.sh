#!/bin/bash
# openssl 独立测速探针构建(独立于 src/ 指令探针矩阵, 可单独执行)
# 静态链接同形态 libcrypto.a, 产 dist/bin/openssl-{x64,i386}_linux(全静态 ELF)
...
echo "== build dist/bin/openssl-x64_linux"
build_one "$DIST/openssl-x64_linux"
check_static "$DIST/openssl-x64_linux"

echo "== build dist/bin/openssl-i386_linux"
build_one "$DIST/openssl-i386_linux" -m32
check_static "$DIST/openssl-i386_linux"
