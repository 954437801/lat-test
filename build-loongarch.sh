...
#
# 用法:
#   bash build-loongarch.sh                 # 默认编 loongarch 在册组(cfloat cint)
#   bash build-loongarch.sh cfloat          # 指定组(可多个, 空格分隔)
#   GRPS="cfloat" bash build-loongarch.sh   # 同上的环境变量写法
#   仅准备工具链、不构建:  bash build-loongarch.sh --toolchain-only
#
# 可用环境变量覆盖:
...
#
# 参照: 13-湖北大机段/电子签章驱动-书生科技/npapi-agent/pack/build.sh 的 check_toolchain()
# (那边探测 g++, 本工具全 C, 改探测 gcc)。
set -euo pipefail
...
# ---- 参数: 支持位置参数覆盖 GRPS 与 --toolchain-only ----
TOOLCHAIN_ONLY=0
pos=()
for a in "$@"; do
    case "$a" in
        --toolchain-only) TOOLCHAIN_ONLY=1 ;;
        -*) echo "未知参数: $a (支持 --toolchain-only)" >&2; exit 1 ;;
        *) pos+=("$a") ;;
    esac
done
if [ "${#pos[@]}" -gt 0 ]; then
...
setup_toolchain() {
    ...
    # 解 .tar.xz: 优先系统 xz; 缺 xz(如精简 WSL 无 xz-utils 且无法 sudo apt)则退回
...
if [ "$TOOLCHAIN_ONLY" = 1 ]; then
    ...
    echo "后续可直接: PATH=\"$TOOLCHAIN_DIR/bin:\$PATH\" ./build.sh --groups=\"$GRPS\""
    exit 0
fi
...
rc=0
for g in $GRPS; do
    b="dist/bin/${g}-loongarch64_linux"
    if [ -f "$b" ]; then
        echo "  [OK] $b"
