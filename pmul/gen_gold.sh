#!/bin/bash
# gen_gold.sh —— 回填 pmul 组真值表: 调**本组探针自己的** --gen-gold, 自检全过才回填
#
# 用法:
#   bash pmul/gen_gold.sh                  # 生成 + 自检 + 回填 + 复编 + 闭环冒烟
#   bash pmul/gen_gold.sh --ver mytag      # 换版本串(随 meta 行 pmul_ver 入库)
#   bash pmul/gen_gold.sh --dry-run        # 只生成并自检, 不动工作区的表
#
...
gcc $CF -Wall -o "$TMP/gengold" "$S/isb_pmul.c"
echo "   编译过(告警数=$( { gcc $CF -Wall -c "$S/isb_pmul.c" -o "$TMP/neg.o"; } 2>&1 | grep -c warning: || true ))"
...
if ! (cd "$T" && ./build.sh --groups=pmul > "$TMP/build.log" 2>&1); then
