...
('sfloat', '软浮点(SoftFloat-3e)延迟与正确性', 'Berkeley SoftFloat-3e 三宽度 f64(float64_t)/f80(extF80)/f128(float128_t) 各做 add/sub/mul/div/sqrt 的单依赖链延迟, 另加 2 条 sin 多项式(仅测速, 无真值) = 17 用例; 并对 8 个固定槽逐位校验 SoftFloat 结果与真值表(真 x86 采集: f64=SSE double, f80=x87 long double, f128=GCC __float128 独立软实现/非硬件真值)。三形态 i386/x64/loongarch64 共存: loongarch64 为原生基线, x86 为 under-LATX 对照。自包含独立探针(只依赖 ib_fields.h, 不引 x86 专有的 ib_core.h), 功能判定走 verify_sfloat(func 段), --debug 出 debug_sfloat(dbg 段)', 'run_id,abi,grp,cname,rep_n + 动态度量列(latency_ns); verify_sfloat 另存本组每功能(true/false)');
