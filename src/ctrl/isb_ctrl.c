 ...
 *
 * 用例来源 = 实测 三层映射 round5_form_weights.csv 里栈类的实测 k90 形态 +
 ...
 *   - pusha/popa/_BOUND 本次不登记: 三者在 64 位长模式下是**无效操作码**
 ...
 *     i386 视角, 本次以实测 k90 为准, 需要时可另开 i386 专有组。
 *
 ...
 *   - 本组实测结论(四重证据齐): 6 条 kat_st=OK 8/8(四形态零告警)
 *     + kat_audit.py 审=6 通过=6(栈族规则在那个独立第二实现里单独写) + 负向注入
 *     9 处全检出 + x86_64 参考机 6 行 OK、零 KATFAIL、零 SIGILL。
 *   - 计时签名撞车(不视为缺陷, 口径见 docs/设计/功能测试与KAT.md):
 *     签名重复自检实测 push m.r.q-rsp / pop r.m.q-rsp / pop m.m.q-rsp 三条 lat_sig
 ...
 *     「本 ABI 无接 KAT 的行」而不是 KATFAIL(见 校验脚本)。
 */
#include "ib_core.h"
#include "ib_buf.h"
