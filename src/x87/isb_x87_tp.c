 ...
 *    不是只按助记符直调): 探针 = x87 位段校验探针 的 A-H 八节, 一条 asm 都不写,
 *    双机 139 行可比 / 差异 0 / 一列都不必剥(P1、P2 的形状没有 P7 那种 fenv 分家)。
 *    通道脚本已泛化(校验脚本{build,pe,参考机,two_diff,dis}.sh + x87_p1chk.strip 边车,
 *    剥列规则必须带实测出处 —— 本探针的边车里一条 S 规则都没有, 因为两机逐字可比),
 *    逐函数指令归因看 反汇编记录{exe,elf}.txt(每节都坐实到具体助记符:
 ...
 * (special 原注在此处写过两条"实测与文档不一致"的事实, 现已由实测/2 订正:
 ...
 * 进算术就把档的行为(PC 影响位宽, 实测 g2)混了进来 —— 一条 case 同时回答两个问题,
 ...
 *  个前提来自伪实测(见文件头 g0), 已被两台真机实测否掉: 三档位形互异且与 SDM 一致,
 ...
 *    位段一律按实测/6 的真机读回: **TOP = bits 11-13(LSB=bit11)** —— 早期设计
 ...
 *    "舍入没丢位"混进同一个 sw 读数。
 *    sw 读数的位置: 一律落在**任何存回/弹栈之前**(实测 e) 末段: 存回类指令会把 C1
 ...
 *    一条贯穿全节的硬件事实(实测 c)/G3): ST(k) = 物理格 (TOP+k)&7、压栈 TOP-1、
 ...
 *    所以 x87 位段校验探针 不接 P5; 本节六条仍只有内核一路, 判据靠实测 的双基准。
 * ===================================================================== */

/* ---- stack_top_after_fild: 压 n 次(n=kk+1)读峰值 TOP, 再弹 n 次读回归零 ---- [解析]
 * 期望 o0 == ((8-n)&7)<<11: 实测 c) 实测 finit 后 TOP=0、每次压栈 -1、第 1 次读回
 ...
 * 就是实测 的 G2 上表: 8 压之后 sw=0x0000(TOP 走完一圈回 0 且**无溢出** —— 第 9 次
 ...
 * 实测 d)/G3 的形状: ST0 <-> ST(3) 互换而 TOP 不变。八行换的是**栈上的值**(整条
 ...
 * 早期设计的括注写的是 FSTP、词干名写的是 fldst: 实测 d)/G3c 里这两个动作是
 ...
 * 实测 c): fincstp 每步 +0x0800 且 8 步回环, fdecstp 反向、从 0 退到 7。本条**不发
 ...
 * 实测 c) 实测: 每寄存器两位、物理格 p 在 bit 2p、置 11 = 空; 而这件事在**状态字
 ...
 * "助记符必须是字面量"两件事同时成立(实测 c) 用 4 项栈反证过这个映射:
 ...
 *   补零, 都只会在某一种读法上错(实测 a) 里"fnstsw m2byte 与 fnstsw ax 两路同字"
 *   就是本条的两个锚点, 它同时钉住了状态字的**位序**(写错位序则三路会一起错、但
 *   与 o0 的期望对不上)。
 * 期望: o1 == 0(两个 XOR 都是 0), 而 o0 == 本档的期望 sw(表在 x87_dirty 注里)。
 * C 侧锚点 = x87 位序校验探针 的 G0/G0b/G1 三行(入口默认值、归零后同字、同一脏态
 *   重复读同字 + fnstenv 镜像与 fnstsw 掩码两条路径在低 6 位同字); AX 形那两路无对应物,
 *   见节头 ②。
 * 16 位里有两位在本组结不到非零值: bit7(ES) 与 bit15(B)。实测 a) 实测 masked 路径
 ...
 * 早期设计写的是"CW=0x037f、SW=0x4020、tag 全空"; 其中的 SW=0x4020 已被实测撤回
 * (实测 h ③: finit 与 fninit 之后都是 cw=0x037f / sw=0x0000 / tag=0xffff, b1/b11 双机同
 ...
 * 读序不能倒: tag 只能从 fnstenv 拿(环境头三字段 cw/sw/tw, 实测 b13), 而 fnstenv 存完
 *   **顺手把所有异常屏蔽**(把活动控制字改成全屏蔽形) —— 先取 tag 就把 cw_before 那个
 *   读数洗掉了, 所以档 3 必须 fnstcw 在前、fnstenv 在末尾且不参与 o0。
 * 只登记归零**后**的 tag: 归零前的 tag 要按"哪几个物理格被写过"建模(实测 c) 里
