 ...
 * df /3, 按 SDM 是 FIST m64int 不弹"已被实测/2 订正(见文件头 h 项): DF/7 就是弹栈
 ...
 *    "只改 RC 两位"的往返。位段按实测 g1 的双机实测: **RC = 控制字位 10-11**
 ...
 *    表里不放状态字(SW): 实测 i 只坐实了**正侧**的 C1 读法("量级变大置 1"), 负
 *      侧"向 -inf 舍 => 量级变大"(如 -3.75 在 RD 档)双机都没实测过。拿没验过的方向
 *      约定当[解析]期望, 正是本组最该防的那类错(实测 g0 的成因就是"我以为硬件是这个
 *      意思")。RC 的效应已由 o0 的四档互异签名钉死, 标志一侧留给 P6 连同补实测做。
 *    例外声明: q4bits() 是本文件第二处让 C 的浮点参与取值, 与 qbits() 同式 —— 只做
 *      一次 |q| < 2^24 的整数->double **精确转换**, 不做任何浮点算术, 函数自平衡。
 *    本节的验证通道: x87 位段校验探针 的 H 节(fesetround 四档 x llrint(q/4) 共 32 行,
 ...
 *    本次在这一节抓到的是**自己的期望算错**: rc_expect 初版拿带符号余数 q%4 分档,
 ...
 * 为什么不让屏蔽位也跟着飘: 四档之间唯一的自变量就该是 RC(实测 b: masked 与
...
#define RC_ARM(sn, enc)                                                        \
    static uint64_t k_##sn##_tp(ib_uw iters)                      \
    {                                                                          \
 ...
 *    逐位相同 + 控制字回读一致"), 依据是"现代 Intel(含 参考机)忽略 PC";
 *    实测 g0/g2 的重做实测否掉了这个前提 —— 同一算式三档给出三个互异位形, 于是
 ...
 *    不暴露尾数位数, 所以 x87 位段校验探针 不接 P3(P7 节头那句"库函数不等于指令"
 *    在这里反过来成立: 库没有提供的语义, 就不能假装有第二条路)。
 * ===================================================================== */

/* PC 编码 -> 控制字(位 8-9)。00=24 位、10=53 位、11=64 位(与实测 g2 的实测档位一致)
 ...
 * 指数字段会让两档得到同一个结果、看起来却像"硬件没 honor PC"—— 正是实测 g0 的
 * 形状。两个式子都能用实测 g2 的实测常数反验(2^27+1 = 0x41A0000002000000)。 */
 ...
 *    入值口径全取自实测 e) 的两张实测表(G5/G7a-G7d), 一行为一档、逐行可追溯到
 *    实测记录 里的那一行 —— 本节的期望不包任何未实测档(早期设计的
 *    "ES=1"、sqrt(+dmin) 的无理尾数那两处已按实测换掉, 见文件头 h) ②)。
 *    三条全节约束:
 *      - 全程 CW=0x037f 全屏蔽(X87_BEGIN 的 finit 已钉), 因为实测 a) 实测 unmasked
 *        路径上 #MF 会在**下一条** x87 指令处交付、进程直接带走 —— 早期设计的
 *        "每条出口 fnclex"在这条路径上无事可做(ES 恒 0、挂起异常取不到), 而
 *        fnclex_clears 本身就是量 fnclex 的那一条。
 *      - sw 读数一律取在任何存回/弹栈之前(实测 e) 末段); **两个例外**是
 *        fistp_oe 与 fnclex_clears: 那里的 IE/SF 就是存回/栈操作自己置的, 取早了
 *        读不到那一位(两处的实测行里都没 C1, 所以不掩盖任何位)。
 *      - 字段不够用的时候按 P3 的打包惯例: o1 = 结果 se(低 16) | sw(高 16)。
 *    m80 入值不再手抄十六进制, 全走下面三个式子(每个都能在实测 的实测行里反验):
 ...
 *
 *    C 调用组合交叉验证通道(与内联指令那条完全另一条路): x87 组合校验探针 ——
 *    **一条 asm 都不写**, 除法/开方/乘加/llrint 全交编译器挑指令, 状态字经
 *    fegetenv 读回。双机各 63 行、各自 bad=0, 剥掉工具链列后 diff=0(边车
 *    x87 组合校验探针, 每条剥列规则都带"是哪一拍哪一行让它进来"的出处);
 *    指令归因取 反汇编记录{exe,elf}.txt(objdump -d 逐函数计数)。
 ...
 *      FE-enums 行 参考机 allTC=3d deTC=0 vs PE allTC=3f deTC=2(glibc 的 FE_ALL_EXCEPT
 ...
 *                        两机同字, 只有 Q8 含 DE(=02) -> 实测 那条"DE 跟着操作数需要
 *                        格式转换走"在纯 C 路径上复现; ieTC 分家(PE 0 / 参考机 1)
 *                        -> 边界④
 ...
 *    (PE 读到 0132/4032/4402, 参考机 全 0032/0002/0022), 低 6 位不受影响(fxam 不动
 *    异常位)。本节四处边界的完整措辞在 x87 组合校验探针 文件头。 */

#define X1       0x8000000000000000ULL               /* m80: 1.0 x 2^k 的尾数形 */

/* ---- 超越函数/除法精度边界用例(P8)的两档尾数常量 ----
 * 被测命题: x87 值的有效位超出 double 时 LATX 翻译能否算准。两档共用同一
 * se(同数量级), 唯一分界 = 尾数能否被 double 精确表示(double = 53 位有效 =
 * 1 整数位 + 52 小数位; m80 = 64 位显式尾数):
 *   f64 档: 尾数用满 53 位有效(位 63..11 = 1, 低 11 位 = 0) -> 恰卡在 double
 *           能精确表示的上界, m80<->double 往返无损(对照组: 测 LATX 拿 double
 *           算得对不对);
 *   f80 档: 尾数用满 64 位(位 63..0 = 1) -> 比 double 多出低 11 位, 过 double
 *           必丢那 11 位(越界档)。
 * 反验: MANT_F64 = 2^64-2^11(位 63..11 为 1、低 11 格为 0), 经 double 无损往返;
 * MANT_F80 = 2^64-1 = MANT_F64|0x7FF, 低 11 位 = 1 装不进 double 的 53 位有效位。
 * 两档只差低 11 位(0x7FF), 高位逐位相同 —— 精度差的干净对照。 */
#define MANT_F64 0xFFFFFFFFFFFFF800ULL               /* double 精确档尾数(53 位有效, 低 11 格=0) */

#define MANT_F80 0xFFFFFFFFFFFFFFFFULL               /* 超 double 精度档尾数(64 位有效, 低 11 格=1) */
 ...
 *   空栈 `fxch %st(1)` **也报**(0x0041 = IE|SF 而 TOP 不变) —— 这一档是本次按 SDM 推
 ...
 *    两条与文档相反的事实按文件头 g) 写死在本节(实测 的 G1/G1b: 十四档 n x 两个
 ...
 *   探针 x87 位序校验探针 的 A-G 七节一条 asm 都不写, 由 gcc -m32 -mno-sse -mfpmath=387
 *   自己发 x87; 双基准逐行一致(93 行可比 / 差异 0, 剥列理由写在 校验脚本)。
 *   每条内核都在自己注里标明对应的 C 侧锚点行。覆盖不到的地方写成边界、不假装覆盖 ——
 *   本次实测到三处分界(都是先看到读数不对、再反汇编归因的):
 *     ① CC 位序在 C 侧**没有对应物**: gcc 的比较发的是 fcomip/fucomip(反汇编见
 *        反汇编记录), 两机读到的 sw 里 C3C2C0 恒为 0(无序列只多一个 IE) ——
 *        与本节 fcom_cc 用的非 COMI 族 `fcoml` 不矛盾(实测 g) 的 b9 里 CC 真在状态字里),
...
/* 本节另加的七个位形(式子与上面同形, 每个都能用 x87 位序校验探针 的自检行反验:
 ...
 * 那两行同时是"PC=64 不等于不舍"的证据(实测 g2 的 (2^27+1)^2 就是本向量的一项)。 */
 ...
 * 实测 G7a 实测八档(早期设计的"负值 -> IE"在这里只成立一半: SDM 的 #IE 判据是
 * **源 < 0**, 所以 -0 与 NaN 都不入判):
 ...
 *     形(fldt), 本条入的是 **double** 形(fldl)。x87 组合校验探针 把两者拆开到位:
 ...
 *   这一行专门把"结果非规格化就置 DE"那条错判据否掉(实测 e))。
 ...
 * 实测 b) 的实测规则(早期设计"清全部 Px 与 ES"在这里被限定):
 ...
 * 规则四句话(实测 的 b9 三路 + 实测 G7e 的无序行, 全按新位序读):
 ...
 * C 侧锚点 = x87 位序校验探针 的 G2 八行(同一张入值表的 lt/gt/eq + 无序行置 IE);
 ...
 * 所以"方向对但操作数抄反"也当场露(锚点 = x87 位序校验探针 的 B1-B4 四行, 双机同字:
 ...
 * 早期设计的词干写的是 m64int, 但架构里**没有 FISUB/FISUBR m64int 形**: FI* 整型族
 *   只有 m16int(DE /4、/5)与 m32int(DA /4、/5)两种宽度, DF 段的 /6 已被 FBSTP 占、/7 已被
 *   FISTP m64int 占(实测 当年订正的就是 DF/7), gas 亦拒 `fisubll`/`fisub qword ptr`
 *   (逐条汇编见 校验脚本 与 校验脚本: `da 26`=fisubl、`de 26`=fisubs、
 ...
 *   浮点再 fsubl, 见 反汇编记录 的 sec_b 直方图), 只有手写 asm 与老二进制才走 DA/4。
 ...
 *   八档全不置 PE(FILD/FISUB 的整型装载按 SDM 无损, m32int 的符号扩展由实测 g) 的
 *   c1/c2 逐值核过)。 */
 ...
 *   被 quiet、bit50 与 bit11 的载荷保留)、QNaN = c000….7fff —— 两行都是 x87 位序校验探针
 *   的 D 节实测值(gcc 发的 fldl, 与本条的内联 asm 无关)。
 * 档 6 同时钉住"FFREE 只改 tag 字节、不动寄存器数据"(实测 c) 只量了 tag 与 TOP,
 ...
 *    尾数是否超出 double(MANT_F64 对照 / MANT_F80 越界), 装载通路两档完全相同
 *    (fldt/fstpt m80)。真值 = 真 x86 硬件 64 位显式尾数结果(gen_val.sh 采集);
 *    若 LATX 把 x87 栈值归一到 double 再算, f80 档的 o0/o1 必偏离真值 -> KATFAIL
 *    直接暴露; f64 档把"精度不足"与"该指令实现整体不同"分开(见计划"已知解读风险")。
 ...
 * ===================================================================== */
struct ib_opinfo {
    const char *stem;    /* 词干前缀(fsin...), 与 _f64/_f80 拼出用例词干 */
...
/* 精度边界用例的 8 组尾数扰动(两档低 11 位的差别就是本质区别):
 *   共享高位(位 63..11): bit63 整数位恒 1, bits62..11 由哈希每 kk 翻若干位(变化量左移
 *     到 bit11 起, 不碰 bit63 与低 11 位) -> 两档同源、同一 kk 高位逐位相同;
 *   f64: 低 11 位置 0 -> 入值恰卡在 double 精确表示的上界(53 位有效), 本档才测
 *     得到"LATX 拿 double 算得对不对"(一旦在 double 装不下的低 11 位上放非 0 值,
 *     两档就都变成"丢那 11 位", 分不出精度不足与实现不同);
 *   f80: 同一高位 | 0x7FF(低 11 位全 1) -> 满 64 位尾数、超 double。
 * 同一 kk 下两档严格只差 0x7FF 这 11 位。注意不能用 x87n(): 它返回 24 位整数
 * (给 qbits 转 double 位形用), 当不了 64 位尾数。 */
...
extern uint64_t k_fild_fistp_q_rt_tp(ib_uw iters);
extern uint64_t k_fild_fistp_d_rt_tp(ib_uw iters);
extern uint64_t k_fild_fistp_w_rt_tp(ib_uw iters);
extern uint64_t k_fild_fistp_q_neg_tp(ib_uw iters);
extern uint64_t k_fild_q_odd_rt_tp(ib_uw iters);
extern uint64_t k_fild_fstp_q_dbl_tp(ib_uw iters);
extern uint64_t k_fild_fstp_t_ext_tp(ib_uw iters);
extern uint64_t k_fistp_q_indef_tp(ib_uw iters);
extern uint64_t k_fisttp_q_rt_tp(ib_uw iters);
extern uint64_t k_fld_t_fstp_t_rt_tp(ib_uw iters);
extern uint64_t k_fbld_fistp_q_rt_tp(ib_uw iters);
extern uint64_t k_fild_fistp_q_x8_tp(ib_uw iters);
extern uint64_t k_rc_fstcw_rt_tp(ib_uw iters);
extern uint64_t k_fld_fstp_tp(ib_uw iters);
extern uint64_t k_fadd_tp(ib_uw iters);
extern uint64_t k_fmul_tp(ib_uw iters);
extern uint64_t k_fistp_tp(ib_uw iters);
extern uint64_t k_fcomip_tp(ib_uw iters);
extern uint64_t k_fcw_rt_tp(ib_uw iters);
extern uint64_t k_stack_top_after_fild_tp(ib_uw iters);
extern uint64_t k_stack_fld8_st8_tp(ib_uw iters);
extern uint64_t k_fxch_st3_tp(ib_uw iters);
extern uint64_t k_fldst_st1_to_st0_tp(ib_uw iters);
extern uint64_t k_fincstp_fdecstp_rt_tp(ib_uw iters);
extern uint64_t k_ffree_tag_tp(ib_uw iters);
extern uint64_t k_fdiv_zero_ze_tp(ib_uw iters);
extern uint64_t k_fsqrt_neg_ie_tp(ib_uw iters);
extern uint64_t k_fistp_oe_tp(ib_uw iters);
extern uint64_t k_stack_underflow_is_tp(ib_uw iters);
extern uint64_t k_denormal_ue_tp(ib_uw iters);
extern uint64_t k_precision_pe_tp(ib_uw iters);
extern uint64_t k_fnclex_clears_tp(ib_uw iters);
extern uint64_t k_fcom_cc_tp(ib_uw iters);
extern uint64_t k_fcompp_cc_tp(ib_uw iters);
extern uint64_t k_fsubr_pair_tp(ib_uw iters);
extern uint64_t k_fisubr_m64_tp(ib_uw iters);
extern uint64_t k_fscale_int_tp(ib_uw iters);
extern uint64_t k_fxam_kinds_tp(ib_uw iters);
extern uint64_t k_fstsw_allbits_tp(ib_uw iters);
extern uint64_t k_fninit_defaults_tp(ib_uw iters);
extern uint64_t k_fadd(ib_uw iters);
extern uint64_t k_fmul(ib_uw iters);
extern uint64_t k_x87_generic_lat(ib_uw iters);
/* P1 精度类专用延迟 */
extern uint64_t k_fild_fistp_q_rt_lat(ib_uw iters);
extern uint64_t k_fild_fistp_d_rt_lat(ib_uw iters);
extern uint64_t k_fild_fistp_w_rt_lat(ib_uw iters);
extern uint64_t k_fild_fistp_q_neg_lat(ib_uw iters);
extern uint64_t k_fild_q_odd_rt_lat(ib_uw iters);
extern uint64_t k_fild_fstp_q_dbl_lat(ib_uw iters);
extern uint64_t k_fild_fstp_t_ext_lat(ib_uw iters);
extern uint64_t k_fistp_q_indef_lat(ib_uw iters);
extern uint64_t k_fisttp_q_rt_lat(ib_uw iters);
extern uint64_t k_fld_t_fstp_t_rt_lat(ib_uw iters);
extern uint64_t k_fbld_fistp_q_rt_lat(ib_uw iters);
extern uint64_t k_fild_fistp_q_x8_lat(ib_uw iters);
/* P2/P3 专用延迟 */
extern uint64_t k_rc_fistp_q_rn_lat(ib_uw iters);
extern uint64_t k_rc_fistp_q_rd_lat(ib_uw iters);
extern uint64_t k_rc_fistp_q_ru_lat(ib_uw iters);
extern uint64_t k_rc_fistp_q_rz_lat(ib_uw iters);
extern uint64_t k_rc_fstcw_rt_lat(ib_uw iters);
extern uint64_t k_pc24_fadd_lat(ib_uw iters);
extern uint64_t k_pc53_fadd_lat(ib_uw iters);
extern uint64_t k_pc64_fmul_lat(ib_uw iters);
/* 迁入/栈/异常/特殊类专用延迟 */
extern uint64_t k_fld_fstp_lat(ib_uw iters);
extern uint64_t k_fistp_m64_lat(ib_uw iters);
extern uint64_t k_fcomip_lat(ib_uw iters);
extern uint64_t k_fcw_rt_lat(ib_uw iters);
extern uint64_t k_stack_top_after_fild_lat(ib_uw iters);
extern uint64_t k_stack_fld8_st8_lat(ib_uw iters);
extern uint64_t k_fxch_st3_lat(ib_uw iters);
extern uint64_t k_fldst_st1_to_st0_lat(ib_uw iters);
extern uint64_t k_fincstp_fdecstp_rt_lat(ib_uw iters);
extern uint64_t k_ffree_tag_lat(ib_uw iters);
extern uint64_t k_fdiv_zero_ze_lat(ib_uw iters);
extern uint64_t k_fsqrt_neg_ie_lat(ib_uw iters);
extern uint64_t k_fistp_oe_lat(ib_uw iters);
extern uint64_t k_stack_underflow_is_lat(ib_uw iters);
extern uint64_t k_denormal_ue_lat(ib_uw iters);
extern uint64_t k_precision_pe_lat(ib_uw iters);
extern uint64_t k_fnclex_clears_lat(ib_uw iters);
extern uint64_t k_fcom_cc_lat(ib_uw iters);
extern uint64_t k_fcompp_cc_lat(ib_uw iters);
extern uint64_t k_fsubr_pair_lat(ib_uw iters);
extern uint64_t k_fisubr_m64_lat(ib_uw iters);
extern uint64_t k_fscale_int_lat(ib_uw iters);
extern uint64_t k_fxam_kinds_lat(ib_uw iters);
extern uint64_t k_fstsw_allbits_lat(ib_uw iters);
extern uint64_t k_fninit_defaults_lat(ib_uw iters);
/* P8 超越函数/除法精度边界(13 指令 x 2 档尾数 = 26 用例, lat/tp/kat 各 26) */
extern uint64_t k_fsin_f64_lat(ib_uw iters);
extern uint64_t k_fsin_f80_lat(ib_uw iters);
extern uint64_t k_fcos_f64_lat(ib_uw iters);
extern uint64_t k_fcos_f80_lat(ib_uw iters);
extern uint64_t k_fsqrt_f64_lat(ib_uw iters);
extern uint64_t k_fsqrt_f80_lat(ib_uw iters);
extern uint64_t k_f2xm1_f64_lat(ib_uw iters);
extern uint64_t k_f2xm1_f80_lat(ib_uw iters);
extern uint64_t k_fsincos_f64_lat(ib_uw iters);
extern uint64_t k_fsincos_f80_lat(ib_uw iters);
extern uint64_t k_fptan_f64_lat(ib_uw iters);
extern uint64_t k_fptan_f80_lat(ib_uw iters);
extern uint64_t k_fpatan_f64_lat(ib_uw iters);
extern uint64_t k_fpatan_f80_lat(ib_uw iters);
extern uint64_t k_fyl2x_f64_lat(ib_uw iters);
extern uint64_t k_fyl2x_f80_lat(ib_uw iters);
extern uint64_t k_fyl2xp1_f64_lat(ib_uw iters);
extern uint64_t k_fyl2xp1_f80_lat(ib_uw iters);
extern uint64_t k_fprem_f64_lat(ib_uw iters);
extern uint64_t k_fprem_f80_lat(ib_uw iters);
extern uint64_t k_fprem1_f64_lat(ib_uw iters);
extern uint64_t k_fprem1_f80_lat(ib_uw iters);
extern uint64_t k_fdiv_f64_lat(ib_uw iters);
extern uint64_t k_fdiv_f80_lat(ib_uw iters);
extern uint64_t k_fdivr_f64_lat(ib_uw iters);
extern uint64_t k_fdivr_f80_lat(ib_uw iters);
extern uint64_t k_fsin_f64_tp(ib_uw iters);
extern uint64_t k_fsin_f80_tp(ib_uw iters);
extern uint64_t k_fcos_f64_tp(ib_uw iters);
extern uint64_t k_fcos_f80_tp(ib_uw iters);
extern uint64_t k_fsqrt_f64_tp(ib_uw iters);
extern uint64_t k_fsqrt_f80_tp(ib_uw iters);
extern uint64_t k_f2xm1_f64_tp(ib_uw iters);
extern uint64_t k_f2xm1_f80_tp(ib_uw iters);
extern uint64_t k_fsincos_f64_tp(ib_uw iters);
extern uint64_t k_fsincos_f80_tp(ib_uw iters);
extern uint64_t k_fptan_f64_tp(ib_uw iters);
extern uint64_t k_fptan_f80_tp(ib_uw iters);
extern uint64_t k_fpatan_f64_tp(ib_uw iters);
extern uint64_t k_fpatan_f80_tp(ib_uw iters);
extern uint64_t k_fyl2x_f64_tp(ib_uw iters);
extern uint64_t k_fyl2x_f80_tp(ib_uw iters);
extern uint64_t k_fyl2xp1_f64_tp(ib_uw iters);
extern uint64_t k_fyl2xp1_f80_tp(ib_uw iters);
extern uint64_t k_fprem_f64_tp(ib_uw iters);
extern uint64_t k_fprem_f80_tp(ib_uw iters);
extern uint64_t k_fprem1_f64_tp(ib_uw iters);
extern uint64_t k_fprem1_f80_tp(ib_uw iters);
extern uint64_t k_fdiv_f64_tp(ib_uw iters);
extern uint64_t k_fdiv_f80_tp(ib_uw iters);
extern uint64_t k_fdivr_f64_tp(ib_uw iters);
extern uint64_t k_fdivr_f80_tp(ib_uw iters);
extern void k_fsin_f64_kat(int kk, ib_kv *g);
extern void k_fsin_f80_kat(int kk, ib_kv *g);
extern void k_fcos_f64_kat(int kk, ib_kv *g);
extern void k_fcos_f80_kat(int kk, ib_kv *g);
extern void k_fsqrt_f64_kat(int kk, ib_kv *g);
extern void k_fsqrt_f80_kat(int kk, ib_kv *g);
extern void k_f2xm1_f64_kat(int kk, ib_kv *g);
extern void k_f2xm1_f80_kat(int kk, ib_kv *g);
extern void k_fsincos_f64_kat(int kk, ib_kv *g);
extern void k_fsincos_f80_kat(int kk, ib_kv *g);
extern void k_fptan_f64_kat(int kk, ib_kv *g);
extern void k_fptan_f80_kat(int kk, ib_kv *g);
extern void k_fpatan_f64_kat(int kk, ib_kv *g);
extern void k_fpatan_f80_kat(int kk, ib_kv *g);
extern void k_fyl2x_f64_kat(int kk, ib_kv *g);
extern void k_fyl2x_f80_kat(int kk, ib_kv *g);
extern void k_fyl2xp1_f64_kat(int kk, ib_kv *g);
extern void k_fyl2xp1_f80_kat(int kk, ib_kv *g);
extern void k_fprem_f64_kat(int kk, ib_kv *g);
extern void k_fprem_f80_kat(int kk, ib_kv *g);
extern void k_fprem1_f64_kat(int kk, ib_kv *g);
extern void k_fprem1_f80_kat(int kk, ib_kv *g);
extern void k_fdiv_f64_kat(int kk, ib_kv *g);
extern void k_fdiv_f80_kat(int kk, ib_kv *g);
extern void k_fdivr_f64_kat(int kk, ib_kv *g);
extern void k_fdivr_f80_kat(int kk, ib_kv *g);
