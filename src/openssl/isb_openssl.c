/*
 * openssl_probe.c —— 独立 openssl 测速探针(与 src/ 指令探针无关, 不 include ib.h)
 *
 * 静态链接同形态 libcrypto.a(EVP 接口), 测各加密/摘要算法在固定块尺寸下的
 * encrypt/digest 吞吐。同一 ELF 在 参考机 原生直跑、翻译机 经 LATX 全翻译,
 * 即 "LATX 翻译 x86 openssl 加密实现" 的性能。
 *
 * stdout 契约(3 行, 与 bench 29 列协议区分):
 *   行1 注释: "# openssl_ver=<OPENSSL_VERSION_TEXT>"
...
static int g_dbg_err = 0;   /* 环境变量 OPENSSL_PROBE_ERR=1 时 fail 路径打印 ERR 栈 */
