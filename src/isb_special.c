 ...
 * fcomip / fcw_rt)现在是 src/isb_x87.c 的同名词干(内核逐字搬、词干与用例名不变,
 ...
 * 测的是**指令功能**, 不去对齐 xperf 表里的形态(尺寸/寻址段/lock 前缀的组合):
 ...
 * 实测事实(i386-windows 复用 x86_64 采的表, _tmp/g5i.sh): 用自然宽度 I64 时
 ...
 *    实测事实(x86_64 参考机, _tmp/spxadd.sh): 零骨架的 xadd 内存 RMW 链报 0.33
 ...
 * 预取/屏障无架构结果 -> lat 填 NULL、不接 KAT; 其余各形都接。bpop/b8 本轮全空
