 ...
 *   查表软模拟要多久」。过程报告《LATX-PCLMULQDQ翻译优化-遍历稀疏侧》第 7 节把
 ...
 *
 * 输出契约(2026-09-10 定案, 不套 IB_ROW_* 模板、不注册 ib_case; 名字全取自
...
/* ==================== 1. 档位表(96 档, 全部由规则生成) ====================
 * 采集模式(2026-09-10 用户定案): **不特意固定任何操作数** —— 按规则生成指定位长
...
/* 两张真二维交叉乘积表。尺寸钉在编译期: 用户定的就是 256 B / 128 KB 这两档,
...
static void pmul_check_gold(void)
{
    ...
    if (IB_PMUL_GOLD_N != PT_MAX || IB_PMUL_GOLD_N != g_pt_n) {
        ...
        fprintf(stderr, "pmul: 真值表 %d 行 / 探针档位 %d 档 / 表上限 %d 不一致\n"
                        "      -> 全部判 TIERMISMATCH; 请用 pmul/gen_gold.sh 重新回填真值表\n",
...
static void pmul_usage(const char *av)
{
    ...
    fprintf(stderr, "            stdout(不写文件, 回填由 pmul/gen_gold.sh 做完形状自检与\n");
 ...
 * 2026-09-10 定案: 真值就是**真机(Windows PE / WSL / box31)上硬件 pclmulqdq 采进
 ...
 * 输出是完整的 .h 文本(含注释/guard/N), 由 pmul/gen_gold.sh 做形状自检后备份回填。
 * 不带日期等易变量: 同一台真机上重跑必得逐字节相同的文件。 */

/* 真机守卫: 真值表只能由真 x86 硬件采集。
 * LATX 下(box22)跑的是同一份 x86_64 ELF、编译期宏完全一致, 只有内核机器名会
...
{
#ifdef _WIN32
    return 1;   /* PE 侧只在真 x86 Windows 上跑; 本仓 gen-gold 只走 WSL 通路 */
#else
    struct utsname u;
...
static void pmul_gen_gold(void)
{
    ...
    printf("/* isb_pmul_gold.h —— pmul 组 %d 档「64x64 -> 128 无进位乘」输入输出真值表"
           "(生成物, 请勿手改)\n *\n", PT_MAX);
    printf(" * 由本组探针自己生成: pmul/gen_gold.sh 调 %s --gen-gold,\n", "isb_pmul");
    printf(" * 在真 x86_64 机器上用硬件 pclmulqdq 直算 96 档(ctrl=0x00、低 64 位放操作数)。\n");
    printf(" * **这份采集值就是判定标准**(2026-09-10 定案): 测试时实测 != 本表即 st=FAIL,\n");
    ...
    printf(" *\n");
    printf(" * 行尾: 本仓统一 LF(2026-09-10 定案), gen_gold.sh 回填前验 CR 字节数为 0。\n");
