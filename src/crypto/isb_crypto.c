 ...
 * 去重(功能测试标准 §5.1): aes_ecb128 块负载已删 —— 它与 openssl 探针的 aes-128-ecb
 *           观测面重叠且 openssl 是真实库实现, 库级 ECB 归 openssl; 本组只留“单指令”延迟/吞吐。
 * 输出契约: kat 三例走 ib_kat(PASS/FAIL + detail); 其余 lat/tput 双内核。
 * 编码纪律: target("aes"/"pclmul"/"sha") 单启 -> legacy 66 0F 38 编码
 *           (忠实真实 32 位 x86 应用负载); 密钥经全局 g_rk 运行时展开, 链不可闭式折叠。
 */
#include "ib_core.h"
