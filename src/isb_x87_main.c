 ...
 *
 * 为什么要单独立一组(缺陷本体, 见 README「LATX x87 64 位整型搬运」一节与
 * 过程报告第 4 章): LATX 翻译 i386 的 x87 整型搬运时, 把 FILD m64int 按
 ...
 *      真实负载(libcef 等)在 64 位下根本不会执行到 fildll/fistpll, 给 x86_64 造
 *      一份 x87 探针 = 测一条不存在的路径, 且会把"i386 才有"的缺陷读数摊薄成
 *      两 ABI 各半的假覆盖。
 *   2) 原先写在这里的"ISA 事实"(SDM: FLD/FST/FSTP m80real 在 64-Bit Mode 为
 *      Invalid)**不能拿来当护栏** —— 实测(取证 6, 探针 _tmp/x87_abi_cmp.sh):
 *      同一份源用 gcc 不带 -m32 编出 x86_64 ELF, 在本机 WSL 上 fldt/fstpt 照旧
 *      执行(不 #UD、rc=0、164 行齐全), 自报 ok=18/18, 且 144 块 K 行与 box31 的
 ...
 *     —— 取证 g 重做(见下 g0)证明 PC 被硬件 honor, 于是 P2/P3 每一档都能给绝对值
 ...
 *
 * ★ 双物理机实测事实(取证 1-5; 探针 _tmp/x87_enc_probe.c, 两份输出
 *   _tmp/x87_probe_pe.txt / _tmp/x87_probe_box31.txt 由 _tmp/pe31_diff.sh 逐行
 *   比对 = 104 行完全一致; 顺序按门禁: 先 Windows 本机 PE
 *   (i686-w64-mingw32-gcc -O2 -static), 后 box31 native ELF(gcc -m32 -O2 -static)):
 ...
 *      "PC 被硬件忽略 => P3 只能走[档间]", 那个结论是**伪取证**得到的, 重做过程与
 *      教训全在下面 g0): 现按实测订正。
 *      g0) 为什么重做: 旧探针 _tmp/x87_enc_probe.c 的 pc_probe() 里 `andl $0xffffc3ff`
 ...
 *          旧日志 _tmp/x87_probe_pe.txt 自己就写着这件事(三行全 `(cw 037f->037f)`),
 *          但它打印的是**我们写进内存的数**、不是 fnstcw 的硬件读回, 所以当场没被
 *          发现。重做探针 = _tmp/x87_cw_probe.c(三节: RC 定位 / PC 三档 / 可写位档),
 *          两份输出 _tmp/x87_cw_pe.txt(本机 PE) 与 _tmp/x87_cw_box31.txt(native ELF)
 *          去掉 CR 后**逐字节相同**(PE 侧每行带 \r 是 mingw 文本模式的 stdout, 属通道
 *          不是数据差); 且两边各数出 38 条 x87 指令、次序一致(_tmp/x87_cw_dis.sh) ——
 ...
 *          算术)三档恒同 => 旧取证 5 那半句"装载/存出不受 PC 影响"仍成立, 只是当时
 ...
 *      ★ 对本轮判据的连带后果(不写清就会被当成随手改口径): 判据三分界把 P3 划给
 *        [档间]、依据是"现代 Intel 硬件忽略 PC、恒用 80 位内部精度", 而该前提在
 *        计划点名的那台机器(box31 i3-4150)上已被上面 g2 的实测否掉。所以 P3 三条
 ...
 *   i) FST 到 m64real 的舍入会把方向记进状态字(取证 a 的位序在此直接可用): 两台
 ...
 *      直接点名要新取证, 不拿没验过的"数轴方向"读法替硬件说话。
 *
 * 取证 6(探针 _tmp/x87_abi_cmp.sh, 与上面 1-5 不是同一件事: 那五组是"指令行为",
 *   这一组是"登记口径"): 把同一份源用 gcc 不带 -m32 编成 x86_64 ELF 在本机跑 ——
 *   rc=0、164 行齐全、T=18 / K=144、自报 ok=18/18, 且 144 块 K 行(13 列全字段)
 *   与 box31 上跑的 **i386 ELF** 逐字相同(差异 0 块)。两个直接结论:
 ...
 *
 * 取证 7(B3 前置: P5 栈 / P6 异常 / P7 特殊 的全部空档) —— 两份探针、两次双基准:
 *   _tmp/x87_fx_probe.c(异常延迟上报 + 栈/标记/环境 + 组合方向, 按 mode 分进程),
 *     输出 _tmp/x87_fx_pe.txt 与 _tmp/x87_fx_box31.txt 各 203 行, 去掉 CR 后逐字节相同;
 *   _tmp/x87_c_probe.c(G1..G7), 输出 _tmp/x87_c_pe.txt 与 _tmp/x87_c_box31.txt 各 366 行,
 *     同样逐字节相同; 且 G7 是**双基准成立之后**补的一节, 补档前那 245 行旧读数在新拍里
 *     逐行原样还在(_tmp/b3_regress_both.sh 的子序列判据 = 0, 两侧各跑一遍) —— 因为
 *     补档时顺带修了打印器 f80() 在非规格化档的一个真错, 而旧读数与它共用打印器。
 *   下面每条都是实跑读数(括号为文件里的行标), P5/P6/P7 的期望只能从这里取。
 *   a) **异常只能走 masked 路径**(计划 103/105 行两处口径按实测改):
 *      清 ZEM 后真造一次 1/0: 出错指令**自身不死**(打印了"div 之后还活着"), fnstsw 也不
 *      死且读回 0xb884(ZE=1、ES=1、B=1、TOP=7) => 计划 185 行问的"ES 后接 fnstsw 会不会
 *      转成 #MF"答案是不会; 但**下一条 x87 指令 fld1 就没回来** —— 该 mode 的输出正好截
 *      在 fld1 之前那行, 而 stdout 是 _IONBF 无缓冲、box31 侧同一处截断。另一路把 IEM 清掉
 ...
 *      IE 与 SF 清了, **C1 仍留着**。计划 103 行"清全部 Px 与 ES"因此限定为"清 Px 与 ES、
 ...
 *      IE|SF 而 TOP 不变) —— 这一档是本轮按 SDM 推"fxch 不压不弹故不报栈错"被实测推翻的。
 ...
 *      向上舍**(1/3 有、1/7 无、舍到 +Inf 有) => 取证 i 的"负侧未实测"仍不写期望, 但正侧
 *      这条已从[采集]升为可推导。
 *      另有一条**只约束读数位置、不写成期望**的机制: 同一次运算的 fnstsw 取在存回指令
 *      之前还是之后, C1 会不一样 —— G7d 的 2^16383*2 是 运算后 0x3a28 / 弹后 0x0028,
 *      取证 6 的 a6 同形(post-store 无 C1), 而 G7a 的 sqrt(+dmin) 是 fsqrt后 0x3822 /
 ...
 *      ① 计划 99 行的位序"TOP=bits 8-10、C1=bit11、C0/C2/C3=bits 12-14"错, 真序见取证 a)
 *        (本文件上面的 a) 条); 探针打印器把两种读法并排打(TOP11_13 与 TOP8_10), 后者在有
 *        CC 位的行里读出垃圾(0x3a41 -> TOP8_10=2), 这张自证表在 _tmp/x87_fx_pe.txt 每行里。
 *      ② 计划 103 行 fdiv_zero_ze 的"ES=1"在 0x037f 全屏蔽路径上取不到(见 a)), 改成断言
 *        ZE=1 + 结果 ±Inf 位形 + ES 恒 0; fnclex_clears 的"清全部 Px 与 ES"按 b) 限定。
 *      ③ 计划 109 行 fninit_defaults 的"SW=0x4020"不成立: finit 与 fninit 之后都是
 ...
 *        double 的隐含整数位读了, 原始记录在 _tmp/x87_c_probe.c 头注, 重推后 c1/c2/c3 与
 *        (b-a, a-b, a-b, b-a) 逐行吻合。计划 192 行给 P7 的 FSCALE 划"采集 golden", 现按
 *        g) ① 改走[解析](这是**收紧**不是放宽: 采集值仍填表, 审计端多一条独立断言)。
 *
 * 实测编码表(gas -m32 逐条汇编 + objdump, 见 _tmp/gas_mat.sh; 十六进制为
 ...
 *     0x7fffffff/INT32_MIN 这两对边界(取证 3c/3b 的入值就是它)。
...
/* 编译期硬拦: 本组只允许 i386(理由与实测取证见文件头「为什么只在 i386 注册」)。
...
#if !defined(__i386__)
#error "isb_x87.c 只在 i386 编译: x86_64 的 psABI 用 SSE2 做浮点, 不发射 x87 整型搬运, 该探针在 x86_64 上测的是不存在的路径(误登记还全绿, 见文件头取证 6)"
#endif
 ...
 * (取证 4f), 所以全部 < 1e18; 里面既有 >=2^53 的(6 个)也留一个 <2^53 的对照。 */
 ...
 * 索引 cw@0 sw@2 tw@4 = 字节偏移 0/4/8(32 位模式的 28 字节布局, 取证 6 的 b13 核过) */
 ...
 * 而取证 7 e) 要求所有 sw 读数都必须落在存回之前 —— 一个槽装不下这种形状。 */
 ...
 * 状态字里, 只能从环境块取(取证 6 的 b13: 环境头三字段就是 cw/sw/tw)。 */
 ...
 * 字节 bit7 负号(取证 4f 实测布局)。入值已由 BVEC 限定 < 1e18, 不校验截断。 */
 ...
 * (与 _tmp/x87_enc_probe.c 里已被两台机器验过的 ext_expect() 同式) */
 ...
 *      fcomip   -> _tmp/x87_p7chk.c 的 G2 八行。反汇编坐实 cc_after 发的就是
 *                  fcomip/fucomip + seta —— 它同时是 P7 那条"比较结果只进 EFLAGS、
 *                  不在状态字里留 CC"的唯一证据行, 两条内核共用一次归因。
 *      fcw_rt   -> _tmp/x87_p1chk.c 的 H2 行末 cwTC 列(glibc 的 __fesetround 体内
 ...
 * 都能对回取证 c)/d)/e)/g) 里具体哪一行(与 fnclex_clears 的 switch 同形, 差别只两处:
