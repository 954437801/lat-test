 ...
 *
 * 用例来源 = xperf 三层映射 round5_form_weights.csv 里 cmp/test 的 k90 形态
 ...
 *
 * 正确性从哪来(本轮定案, 不再靠上面那套链形): 每条 case 另有一具 KAT 探针
 ...
 *     落 stdout, _tmp/gen_val.sh 收割后回填 -> x86 真机只需跑一次;
 *   - 表已回填 -> kat_st=OK/KATFAIL; 龙芯 LATX 等被测机不需采集、不需对拍。
 * 因此骨架/链形如何设计只影响计时, 不影响正确判定。
 *
 * 计时签名撞车(本轮改判: 不再当缺陷, 口径见 README「KAT 真值表」节):
 * q_dup.sh 实测 18 条里 lat_sig 只剩 11 个唯一值、tput_sig 14 个, 几组都是
