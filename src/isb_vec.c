 ...
 *
 * 口径(与 special 组同为定案后的直写风格): 一条指令 = 一个定时长函数 + 一个
 * lat 函数(+ 高危族一个 KAT 探针), 不引参数化模板宏, 不为对齐 xperf 的
 ...
 * 计数走寄存器形(xmm, xmm)而不是立即数形: 前者才是 libcef 里的常见形制。每条的
 ...
 * vpaddq)已迁入 isb_avx.c —— 归位口径: avx = 全部 VEX 编码(见 功能测试标准.md §5.3)。 */
