...
- 一句话：真值只来自真 x86（x86_64 ELF + Windows 本机 PE），同一输入在
...
- 输入不是运行时 rand：`IB_KIN8` 由（词干, 序号）经 FNV-1a + splitmix64 推导，
  ...
  判据。每指令 8 组输入也是 8 连块（block8）的数据源。
- 探针的栈纪律（实测坑）：`IB_SETF/IB_GETF` 用的是 `pushq/pushfq`，写的正好是 `[rsp-8]`，
  ...
  把 i0/o0 全写成 0；该用例现已迁至 x87 组、词干仍叫 `fcw_rt`）。
  口径：用了这两个宏的探针，操作数槽
...
- 为何计时签名不够、还要 KAT：计时内核的 sig16 是 N 轮链累加后的末值，一条
  ...
  操作数/哪个标志直接点名。
- 判据分层（设计取舍，影响所有组的读法）：正确性 = KAT 逐字段比对；计时列只
  要求跨 run/跨机可复现 —— 「两条 case 同签名」不再当交付缺陷（签名重复自检
  从门槛降为诊断工具），因为同签名大多只是「同骨架下值流本就
...
- 签名纪律：数据确定性（非随机）-> sig16 跨 run/跨机逐位可复现；链初值
  ...
  16 轮同增量、只差初值，sig16 会数学归零（实测恒 0000）丧失区分度。
- 示例实测（ns/块，三环境对照）：

  ```
  x86_add_r64: x86_64 0.83(构建机) 1.45(native 机) 1.62(LATX 机);
               i386 3.11(native 机) 5.05(LATX 机)
  sse2_paddd : x86_64 2.9(构建机) 4.34(native 机) 6.33(LATX 机);
               i386 5.48(native 机) 11.97(LATX 机)
  ```

  sig16 跨构建机/native 机/LATX 机逐位一致：`x86_add_r64` = `f3e55bbe1634f248`
