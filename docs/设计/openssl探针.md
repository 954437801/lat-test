...
- **形态**：`src/openssl/isb_openssl.c` + `build/build-openssl.sh`（WSL）静态链同形态
...
- **形态**：与 openssl 同源 `src/openssl/isb_openssl.c`（纯 legacy EVP API，1.1.1 兼容），
  `build/build-openssl11.sh`（WSL）自编 openssl 1.1.1w 静态 `libcrypto.a`（源码
  `/tmp/osrc/openssl-OpenSSL_1_1_1w`，下载见 `third_party/get_openssl.sh`），产
...
- **数据**：与 3.x **同一 `openssl` 表共存**，`openssl_ver` 列分版本（`runs.mode` 记
  `openssl11_<mode>`，二进制 `openssl11_*`）；列集合与 3.x 版一致（Debian 3.5.7 静态
