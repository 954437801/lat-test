...
python3 dist/isbench.py run wine sse crypto          # wine 跑 exe(自动 killw)
python3 dist/isbench.py compare 164955 165048        # 库内 key 前缀对拍
python3 dist/isbench.py export 165048 > run.csv      # xlsx 软件直接导入
python3 dist/isbench.py ls --host 3A6000             # 只看该机器结果
ISBENCH_PUSH=http://分析机IP:8770 python3 dist/isbench.py run latx   # 免 scp
```
